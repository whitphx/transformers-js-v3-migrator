...
Generated by transformers-js-v3-migrator"""
            ...
            if hasattr(commit_info, 'pr_url') and commit_info.pr_url and hasattr(commit_info, 'pr_num'):
                ...
                # Update PR with custom title and description
                try:
                    # Update PR title
                    self.hf_api.rename_discussion(
                        repo_id=repo_id,
                        discussion_num=pr_num,
                        new_title=migration_title,
                        repo_type="model"
                    )
                    ...
                    self.logger.info(f"Updated PR title and description for {repo_id}")
                    
                except Exception as e:
                    self.logger.warning(f"Failed to update PR title/description for {repo_id}: {e}")
                    # Continue anyway - PR exists even if title/description update failed
